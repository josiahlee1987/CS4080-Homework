...
void initVM() {
    ...
    // Size of each half (16 MB)
    vm.spaceSize = 16 * 1024 * 1024;

    // Allocate both halves of the heap
    vm.fromSpace = malloc(vm.spaceSize);
    vm.toSpace = malloc(vm.spaceSize);

    // Pointer to the start of the active space
    vm.freePtr = vm.fromSpace;
