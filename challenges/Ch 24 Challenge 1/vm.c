...
static InterpretResult run() {
    ...
    register uint8_t* ip = frame->ip; // <- store the ip directly in a C local variable (register)
...
#define BINARY_OP(valueType, op) \
    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            // Use local register ip instead of frame->ip
            case OP_JUMP: {
            ...
            case OP_CALL: {
                        ...
                        frame->ip = ip; // Store the ip back into the frame in case the call pushes a new frame
                        if (!callValue(peek(argCount), argCount)) {
                            return INTERPRET_RUNTIME_ERROR;
                        }
                        frame = &vm.frames[vm.frameCount - 1];
                        ip = frame->ip; // reload the ip of the CallFrame being returned to
                        break;
            }
            case OP_RETURN: {
                    Value result = pop();
