...
static void forStatement() {
    ...
    // Variables used to keep track of the original loop variable (so the shadow var can be created later)
    int loopVariable = -1; // init
    Token loopVariableName;
    loopVariableName.start = NULL; // init
    ...
    // SHADOW THE VARIABLE
    int innerVariable = -1;
    if (loopVariable != -1) { // If the loop declares a variable...
        // Create a scope for the copy...
        beginScope();
        // Define a new variable initialized with the current value of the loop
    ...
    // COPY BACK FOR MUTATIONS
    if (loopVariable != -1) {
        // Store the inner variable back in the loop variable
